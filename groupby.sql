...
SELECT DISTINCT
	...
	, ship_country
FROM orders
WHERE UPPER(ship_city) LIKE '%BURG';
...
SELECT 
	  country
	, COUNT(DISTINCT company_name) AS CNTRY_COMPANY_CNT
FROM suppliers
GROUP BY country
ORDER BY CNTRY_COMPANY_CNT DESC;
...
SELECT
	  ship_country
	, SUM(freight) CNTRY_TOTAL_FRGHT
FROM orders
WHERE ship_region IS NOT NULL
GROUP BY ship_country
HAVING SUM(freight) > 2750
ORDER BY CNTRY_TOTAL_FRGHT DESC;
